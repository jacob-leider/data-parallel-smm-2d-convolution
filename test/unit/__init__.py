...
def pooling_poss_output_size(input_height, input_width, padding, stride, kernel_height, kernel_width, ceil_mode):
    ...
    if ceil_mode:
        poss_output_height = math.ceil(top_height / bottom_height) + 1
    else:
        poss_output_height = top_height // bottom_height + 1

    top_width = input_width + 2 * padding[1] - kernel_width
    bottom_width = stride[1]

    if ceil_mode:
        poss_output_width = math.ceil(top_width / bottom_width) + 1
    else:
