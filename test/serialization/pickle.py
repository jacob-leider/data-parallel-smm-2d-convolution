...
def runner(module: torch.nn.Module, input_data: torch.Tensor, name: str):
    try:
        ...
        compare_tensors(
            ...
            f'{name} torch orig and torch unpickled, {models.BATCH} samples')
        ...
        compare_tensors(
            sb_out, target,
            f'{name} swap backend, not pickled, {models.BATCH} samples')
        ai3.swap_conv2d(module)
        sc_out = module(input_data)
        compare_tensors(
            sc_out, target,
            f'{name} swap conv2d, not pickled, {models.BATCH} samples')
        ...
        compare_tensors(
            ...
            f'{name} torch unpickled and ai3 swap unpickled, {models.BATCH} samples')
        ...
        compare_tensors(
            ...
            f'{name} torch unpickled and ai3 model unpickled, {models.BATCH} samples')
    finally:
        os.remove('torch.pkl')
