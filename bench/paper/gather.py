...
swap_direct_out = predict_show_time(
    swap_direct, input, "ai3 direct conv2d", times_for_layer)
swap_smm = ai3.swap_backend(orig, {"conv2d": "smm"})
swap_smm_out = predict_show_time(
    swap_smm, input, "ai3 smm conv2d", times_for_layer)
if ipex_found:
    ipex_model = ipex.optimize(orig, dtype=torch.float32)
    ipex_out = predict_show_time(
        ipex_model, input, "ipex conv2d", times_for_layer)
