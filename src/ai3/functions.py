...
def get_conv2d_output_shape(input: torch.Tensor, kernel: torch.Tensor, padding: Sequence[int]) -> tuple:
    # same
    ...
    output_height = input_height + 2 * padding[0] - kernel_height + 1;
    output_width = input_width + 2 * padding[1] - kernel_width + 1;
...
def conv2d(input: torch.Tensor, kernel: torch.Tensor, bias: Optional[torch.Tensor] = None, padding: Union[int, Sequence[int]] = 0) -> torch.Tensor:
    ...
    output_size = get_conv2d_output_shape(input, kernel, padding)
